...
export class UserService {
  ...
  private readonly API_URL = `${environment.userApiUrl}/users`;
  ...
  getUserStats(): Observable<UserStats> {
    ...
    return this.http.get<any>(`${environment.userApiUrl}/admin/users/stats`).pipe(
      map((raw: any) => ({
  ...
  updateUserSubscription(userId: string, subscriptionType: SubscriptionType): Observable<User> {
    // TODO: endpoint PATCH /api/users/{id}/subscription à implémenter côté backend
  ...
  sendEmailToUser(userId: string, subject: string, message: string): Observable<void> {
    // TODO: endpoint POST /api/users/{id}/send-email à implémenter côté backend
  ...
  getUserActivityLogs(userId: string): Observable<any[]> {
    // TODO: endpoint GET /api/users/{id}/activity-logs à implémenter côté backend
    return this.http.get<any[]>(`${this.API_URL}/${userId}/activity-logs`);
  }

  exportUsers(filter?: UserFilter): Observable<Blob> {
    // TODO: endpoint GET /api/users/export à implémenter côté backend
    let params = new HttpParams();
