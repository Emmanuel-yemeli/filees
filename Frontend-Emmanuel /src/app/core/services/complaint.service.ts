...
export class ComplaintService {
  ...
  /** Réclamations déjà assignées à l'agent connecté */
  getAgentQueue(): Observable<Complaint[]> {
  ...
  resolveComplaint(id: string, request: ResolveComplaintRequest): Observable<Complaint> {
    return this.http.put<Complaint>(`${this.BASE_URL}/${id}/resolve`, null, {
      params: new HttpParams()
        .set('resolution', request.resolution)
        .set('resolutionType', request.resolutionType)
    });
  }
  ...
  getUnassigned(): Observable<Complaint[]> {
    return this.http.get<Complaint[]>(`${this.BASE_URL}/unassigned`);
  }
