...
@Component({
  ...
  template: `
    @if (sent()) {
      <div class="success-msg"><mat-icon>check_circle</mat-icon> Candidature envoyée avec succès !</div>
    } @else if (!showForm()) {
      <button mat-raised-button color="primary" (click)="showForm.set(true)">
        <mat-icon>person_add</mat-icon> Candidature spontanée
      </button>
    } @else {
      <div class="apply-form">
        <h4><mat-icon>person_add</mat-icon> Candidature spontanée</h4>
        <mat-form-field appearance="outline" style="width:100%">
          <mat-label>Message de motivation *</mat-label>
          <textarea matInput rows="4" [(ngModel)]="message"
                    placeholder="Présentez-vous et expliquez votre intérêt..."></textarea>
        </mat-form-field>
        <mat-form-field appearance="outline" style="width:100%">
          <mat-label>Lien CV (optionnel)</mat-label>
        ...
        <div class="actions">
          ...
          <button mat-raised-button color="primary" [disabled]="sending() || !message.trim()" (click)="submit()">
            <mat-icon>send</mat-icon> {{ sending() ? 'Envoi...' : 'Envoyer' }}
          </button>
        </div>
      </div>
    }
  `,
  styles: [`
    .apply-form { border: 1px solid #e5e7eb; border-radius: 10px; padding: 16px; margin-top: 8px; }
