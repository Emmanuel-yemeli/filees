...
export class RecommendationModalComponent implements OnInit {
  ...
  loadJobOffers() {
    ...
    this.jobOfferService.getMyJobOffers().subscribe({
      next: (offers) => {
        ...
        this.jobOffers = offers.filter(offer => offer.status === 'OPEN');
        this.isLoadingOffers = false;
      },
      error: (error) => {
  ...
  onSubmit() {
    if (this.recommendationForm.valid) {
      ...
      // Parse IDs to numbers for backend
      const clientId = typeof currentUser.id === 'string' ? parseInt(currentUser.id, 10) : currentUser.id;
      const freelanceId = this.data.freelancer.id;
      const jobOfferId = typeof this.recommendationForm.value.jobOfferId === 'string' 
        ? parseInt(this.recommendationForm.value.jobOfferId, 10) 
        : this.recommendationForm.value.jobOfferId;
