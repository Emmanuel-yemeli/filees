...
export class ClientProjectDashboardComponent implements OnInit {
  ...
  getDaysLabel(milestone: ProjectMilestone): string {
    ...
    const days = this.getDaysUntilDue(milestone);
    if (days < 0) return this.translate.instant('clientProjects.dashboard.daysLate', { count: Math.abs(days) });
    ...
    return this.translate.instant('clientProjects.dashboard.daysRemainingCount', { count: days });
  }
