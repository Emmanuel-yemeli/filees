...
export class ComplaintDetailComponent implements OnInit, OnDestroy {
  ...
  private watchNotifications(complaintId: string): void {
    this.notifSub = this.notificationService.newNotification$.pipe(
      ...
      this.snackBar.open('Le statut de votre réclamation a été mis à jour.', 'Voir', { duration: 4000 });
    });
  }
  ...
  submitRating(): void {
    ...
    this.complaintService.rateComplaint(this.complaint()!.id, rating).subscribe({
      ...
      error: () => {
        this.snackBar.open('Erreur lors de la notation', 'Fermer', { duration: 3000 });
        this.isRating.set(false);
      }
    });
  }
  ...
  reopen(): void {
    ...
    this.advService.reopenComplaint(this.complaint()!.id, { reason: this.reopenReason() }).subscribe({
      next: updated => {
        ...
        this.snackBar.open('Réclamation réouverte.', 'Fermer', { duration: 3000, panelClass: ['snack-success'] });
      },
      error: err => {
        this.snackBar.open(err?.error?.message ?? 'Impossible de réouvrir.', 'Fermer', { duration: 4000 });
        this.isReopening.set(false);
      }
    });
  }
  ...
  canRate(): boolean {
    const c = this.complaint();
    return !!c && c.status === ComplaintStatus.CLOSED && !c.satisfactionRating;
  }

  showNps(): boolean {
    const c = this.complaint();
    return !!c && (c.status === ComplaintStatus.CLOSED || c.status === ComplaintStatus.RESOLVED);
  }
