...
export class ProjectChatComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  ngOnInit(): void {
    ...
    // Load existing messages and connect
    this.loadMessages();
  ...
  sendMessage(): void {
    ...
    if (this.recipientId) {
      const preview = message.content.length > 100 ? message.content.substring(0, 97) + '...' : message.content;
      this.notificationService.createNotification(
        this.recipientId,
        'PROJECT',
        `New message from ${this.currentUserName}`,
        preview,
        this.projectId,
        'PROJECT'
      );
