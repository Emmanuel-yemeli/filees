...
export class NotificationBellComponent implements OnInit, OnDestroy {
  ...
  readonly filterTabs: { key: FilterTab; label: string; count: number }[] = [
    ...
    { key: 'unread',     label: 'Non lu',       count: 0 },
    { key: 'complaints', label: 'Réclamations', count: 0 },
    { key: 'other',      label: 'Autres',       count: 0 }
  ];
  ...
  markAllRead(): void {
    ...
    this.notificationService.markAllAsRead(user.id).subscribe({
      ...
      error: () => {
        // Endpoint indisponible — on conserve la mise à jour locale
        console.warn('[Bell] markAllAsRead : endpoint indisponible, état local conservé');
        this.cdr.markForCheck();
      }
    });
  }
