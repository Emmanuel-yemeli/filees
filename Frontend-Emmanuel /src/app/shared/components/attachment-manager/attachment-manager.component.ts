...
export class AttachmentManagerComponent {
  ...
  onFilesSelected(event: Event): void {
    ...
    if (current.length + input.files.length > MAX_FILES) {
      this.fileError.set(`Maximum ${MAX_FILES} fichiers autorisés.`);
      input.value = '';
      return;
    }
    ...
    Array.from(input.files).forEach(file => {
      ...
      if (!typeOk) {
        this.fileError.set(`Type non autorisé : ${file.name}.`);
        return;
      }
      if (file.size > MAX_FILE_SIZE_BYTES) {
        this.fileError.set(`Fichier trop volumineux : ${file.name} (max ${MAX_FILE_SIZE_MB} Mo).`);
        return;
      }
      if (current.some(f => f.originalName === file.name && f.file?.size === file.size)) {
        this.fileError.set(`Déjà ajouté : ${file.name}`);
        return;
      }
  ...
  downloadFile(file: ManagedFile): void {
    if (file.file) {
      ...
      this.fileError.set(`Impossible de télécharger : URL non disponible pour ${file.name}`);
    }
  }
