  ...
  template: `
    @if (tracking()) {
      <div class="sla-widget" [class.breached]="isBreached()" [class.warning]="isWarning()">
        ...
        <div class="sla-info">
          <span class="sla-label">SLA résolution</span>
          <span class="sla-deadline">
            @if (isBreached()) {
              <mat-icon class="breach-icon">warning</mat-icon> Dépassé
            } @else {
              {{ remaining() }}
            }
          </span>
        ...
        @if (tracking()!.firstResponseBreached) {
          <span class="badge-breach" matTooltip="Première réponse hors délai">1ère rép. !</span>
        }
      </div>
    }
  `,
...
export class SlaTrackerComponent implements OnInit {
  ...
  private computeRemaining(t: SlaTracking) {
    const ms = new Date(t.resolutionDeadline).getTime() - Date.now();
    if (ms <= 0) { this.remaining.set('Dépassé'); return; }
